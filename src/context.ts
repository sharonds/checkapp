...
export function runContextCommand(args: string[]): void {
  if (args.length === 0 || args[0] === "help") {
    console.log("Usage:");
    console.log("  article-checker context add <type> <file>    Add/update a context document");
    console.log("  article-checker context list                 List all saved contexts");
    console.log("  article-checker context show <type>          Show context content");
    console.log("  article-checker context remove <type>        Remove a context");
    console.log("");
  ...
  switch (parsed.action) {
    ...
    case "update": {
      if (!parsed.type || !parsed.path) {
        console.error("Usage: article-checker context add <type> <file>");
        process.exit(1);
      }
    ...
    case "list": {
      ...
      if (contexts.length === 0) {
        ...
        console.log("Add one: article-checker context add tone-guide ./brand-voice.md");
        return;
      }
      console.log("\nSaved contexts:\n");
    ...
    case "show": {
      if (!parsed.type) { console.error("Usage: article-checker context show <type>"); process.exit(1); }
    ...
    case "remove": {
      if (!parsed.type) { console.error("Usage: article-checker context remove <type>"); process.exit(1); }
